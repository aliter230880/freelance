...
/**
 * Distorted particle sphere with mouse interaction.
 * Ported 1:1 from the FlowForge SaaS Landing Page Template (Aura).
 * - IcosahedronGeometry rendered as THREE.Points with custom ShaderMaterial
 * - 3D Simplex noise distortion in vertex shader
 * - Mouse repulsion: particles push outward when the cursor is nearby
 * - Camera parallax follows the mouse a bit
 */
...
const ThreeCanvas = ({
  ...
  useEffect(() => {
    ...
    // Faint orbital rings behind the sphere
    ...
    const createOrbit = (radius, rotation) => {
      ...
      const mat = new THREE.LineBasicMaterial({
        color: 0x27272a, transparent: true, opacity: 0.5,
      });
    ...
    const orbits = [
      createOrbit(5.5, { x: Math.PI / 2, y: 0 }),
      createOrbit(5.2, { x: Math.PI / 3, y: Math.PI / 6 }),
      createOrbit(6.0, { x: Math.PI / 1.8, y: Math.PI / 4 }),
    ];
    ...
    const applyLayout = () => {
      systemsGroup.position.set(offset.x, offset.y, offset.z || 0);
      systemsGroup.scale.setScalar(scale);
    };
